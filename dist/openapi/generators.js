...
const requestBodySchema = (fields, schema) => ({
    ...schema,
    properties: Object.fromEntries(Object.entries(schema.properties ?? {}).map(([fieldName, schema]) => {
...
const generateRequestBodySchema = (config, collection, type) => {
    const schema = entityToJSONSchema(config, removeInterfaceNames(collection.config), // the `interfaceName` option causes `entityToJSONSchema` to add a reference to a non-existing schema
    new Map(), 'text', undefined);
    schema.properties = Object.fromEntries(Object.entries(schema.properties ?? {}).filter(([property]) => !['id', 'createdAt', 'updatedAt'].includes(property)));
    schema.required = (schema.required ?? []).filter(property => schema.properties?.[property] !== undefined);
    if (type === 'patch') {
        schema.required = [];
    }
    return {
